...
public class HobbyWinner implements ItemListener{
	...
	HobbyWinner() {
		...
		win.setLayout(null);
		win.add(question);
		q_result.setBounds(20, 100, 250, 40);
		win.add(q_result);
	...
	private boolean checkAnswer(ItemEvent e) {
		if (question_index < 5) {			
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (result) {
			...
			if (question_index < 5) {
				...
				q_result.setText("End of quiz.. You may close the window now.");
			}			
		}
		else {
			q_result.setText("Please try again");
		}
